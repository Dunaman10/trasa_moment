...
import { Sliders, Check, Info, HelpCircle, Sparkles, BookOpen } from "lucide-react";
...
const PACKAGES = [
    {
        ...
        price: "Rp 1.250.000",
        features: ["1 Fotografer Profesional", "2 Jam Durasi Sesi", "25 Edited Photos", "All Raw Files (Google Drive)", "1 Lokasi / Studio"],
        color: "from-amber-600 to-amber-700 bg-amber-500/10 text-amber-700 border-amber-200",
    ...
    },
    {
        ...
        price: "Rp 2.750.000",
        features: ["1 Fotografer + 1 Asisten", "4 Jam Durasi Sesi", "50 Edited Photos", "USB Drive Flashdisk Exclusive", "Cetak 1 Rangka Ukuran 16R", "Max. 100 Tamu Undangan"],
        color: "from-slate-400 to-slate-500 bg-slate-500/10 text-slate-700 border-slate-200",
        badge: "Paling Populer untuk Lamaran",
        bgGradient: "from-slate-600 via-slate-700 to-slate-800",
        textColor: "text-slate-200",
        btnColor: "bg-slate-550 hover:bg-slate-500 bg-slate-600 text-white shadow-slate-900/30"
    },
    {
        id: "gold",
        ...
        price: "Rp 4.900.000",
        features: ["2 Fotografer + 1 Videografer", "8 Jam Durasi Sesi (Full-day)", "100 Edited Photos", "Exclusive Photobook Album", "Cinematic Video Highlight (3-5 menit)", "Cetak 2 Rangka Ukuran 16R", "Max. 500 Tamu Undangan"],
        color: "from-yellow-500 to-yellow-600 bg-yellow-500/10 text-yellow-700 border-yellow-200",
        badge: "Pilihan Terbaik Pernikahan",
        bgGradient: "from-amber-600 via-yellow-600 to-amber-800",
        textColor: "text-yellow-200",
        btnColor: "bg-amber-500 hover:bg-amber-400 text-white shadow-amber-950/30 font-bold"
    },
    {
        id: "platinum",
        ...
        price: "Rp 8.500.000",
        features: ["3 Fotografer + 2 Videografer (Multicam)", "Durasi Sesi Fleksibel (Hingga 12 Jam)", "200 Edited Photos + All Raw Files", "Premium Box & 2 Exclusive Photobooks", "Cinematic Wedding Film (10-15 menit)", "Same Day Edit Video Clip Teaser", "Liputan Drone / Aerial Videography", "Tamu Undangan Tanpa Batas"],
        color: "from-cyan-600 to-cyan-700 bg-cyan-500/10 text-cyan-700 border-cyan-200",
...
const evaluateMembership = (budget, scale, duration) => {
    ...
    // Fuzzification
    // 1. Budget membership
    const uBudgetTerbatas = budget <= 1500000 ? 1 : budget >= 3000000 ? 0 : (3000000 - budget) / 1500000;
    const uBudgetMenengah = budget <= 1500000 || budget >= 5000000 ? 0 : 
                            budget <= 3000000 ? (budget - 1500000) / 1500000 : (5000000 - budget) / 2000000;
    const uBudgetTinggi = budget <= 3500000 ? 0 : budget >= 5000000 ? 1 : (budget - 3500000) / 1500000;

    // 2. Scale membership
    const uScaleIntimate = scale <= 3 ? 1 : scale >= 6 ? 0 : (6 - scale) / 3;
    const uScaleStandar = scale <= 2 || scale >= 8 ? 0 : 
                          scale <= 5 ? (scale - 2) / 3 : (8 - scale) / 3;
    const uScaleGrand = scale <= 5 ? 0 : scale >= 8 ? 1 : (scale - 5) / 3;

    // 3. Duration membership
    const uDurationSingkat = duration <= 3 ? 1 : duration >= 6 ? 0 : (6 - duration) / 3;
    const uDurationReguler = duration <= 2 || duration >= 9 ? 0 : 
                            duration <= 5 ? (duration - 2) / 3 : (9 - duration) / 4;
    const uDurationSeharian = duration <= 6 ? 0 : duration >= 9 ? 1 : (duration - 6) / 3;
    ...
    const scoreBronze = Math.min(uBudgetTerbatas, uScaleIntimate, uDurationSingkat);

    // Rule 2: IF Budget is Menengah AND Scale is Intimate/Standar AND Duration is Singkat/Reguler THEN Package is Silver
    const scoreSilver = Math.min(uBudgetMenengah, Math.max(uScaleIntimate, uScaleStandar), Math.max(uDurationSingkat, uDurationReguler));

    // Rule 3: IF Budget is Menengah/Tinggi AND Scale is Standar AND Duration is Reguler/Seharian THEN Package is Gold
    const scoreGold = Math.min(Math.max(uBudgetMenengah, uBudgetTinggi), uScaleStandar, Math.max(uDurationReguler, uDurationSeharian));
    ...
    if (totalScore === 0) {
        ...
        const bDist = Math.abs(budget - 1250000) / 10000000 + Math.abs(scale - 2)/10 + Math.abs(duration - 2)/12;
        const sDist = Math.abs(budget - 2750000) / 10000000 + Math.abs(scale - 4)/10 + Math.abs(duration - 4)/12;
        const gDist = Math.abs(budget - 4900000) / 10000000 + Math.abs(scale - 6)/10 + Math.abs(duration - 8)/12;
        const pDist = Math.abs(budget - 8500000) / 10000000 + Math.abs(scale - 9)/10 + Math.abs(duration - 10)/12;
    ...
    const maxVal = Math.max(weights.bronze, weights.silver, weights.gold, weights.platinum);
...
export default function FuzzyWidget({ onSelectPackage }) {
    ...
    const [scores, setScores] = useState({ bronze: 0, silver: 0, gold: 0, platinum: 0 });
    ...
    useEffect(() => {
        fetch("/api/recommend", {
            ...
            body: JSON.stringify({ budget, scale, duration }),
        })
            .then((res) => res.json())
            .then((data) => {
                if (data.scores) {
                    ...
                    let highest = "bronze";
                    let maxVal = -1;
                    Object.entries(data.scores).forEach(([pkg, score]) => {
                        if (score > maxVal) {
                            maxVal = score;
                            highest = pkg;
                        }
            ...
            .catch((err) => {
                console.error("Fuzzy recommendation failed", err);
    ...
    return (
        <section id="rekomendasi" className="py-20 bg-slate-50 relative overflow-hidden">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                ...
                <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
                    ...
                    <p className="text-slate-600 text-base sm:text-lg">
                        Gunakan kalkulator pintar kami untuk menemukan paket dokumentasi yang paling sesuai dengan kebutuhan Anda secara otomatis.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-5 bg-white p-6 sm:p-8 rounded-3xl shadow-xl border border-slate-100/50 space-y-8">
                        <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
                            ...
                            <div>
                                <h3 className="font-bold text-lg text-brand-deep">Parameter Input</h3>
                                <p className="text-xs text-slate-400">Atur slider di bawah sesuai kebutuhan</p>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center text-sm font-semibold text-slate-700">
                                <span className="flex items-center gap-1.5">
                                    Anggaran (Budget)
                                    <HelpCircle className="w-4 h-4 text-slate-300 hover:text-slate-500 cursor-help" title="Rentang biaya yang direncanakan" />
                                </span>
                                <span className="text-brand-primary font-bold">{formatIDR(budget)}</span>
                            </div>
                            <input
                                type="range"
                                min="1000000"
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center text-sm font-semibold text-slate-700">
                                <span className="flex items-center gap-1.5">
                                    ...
                                    <HelpCircle className="w-4 h-4 text-slate-300 hover:text-slate-500 cursor-help" title="Besaran jumlah tamu dan kerumitan manajemen sesi" />
                                </span>
                                <span className="text-brand-primary font-bold">
                                    {scale <= 3 ? "Intimate (Kecil)" : scale <= 7 ? "Standar (Sedang)" : "Grand (Besar)"}
                                </span>
                            </div>
                            <input
                                type="range"
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center text-sm font-semibold text-slate-700">
                                <span className="flex items-center gap-1.5">
                                    Durasi Liputan (Jam)
                                    <HelpCircle className="w-4 h-4 text-slate-300 hover:text-slate-500 cursor-help" title="Lama waktu sesi foto / liputan di lokasi" />
                                </span>
                                <span className="text-brand-primary font-bold">{duration} Jam</span>
                            </div>
                            <input
                                type="range"
                                min="1"
                        ...
                        <div className="bg-brand-light/20 border border-brand-accent/20 rounded-2xl p-4 flex gap-3">
                            ...
                            <p className="text-xs text-brand-dark/90 leading-relaxed">
                                Rekomendasi dihitung secara langsung berdasarkan kecocokan antara alokasi anggaran, skala acara, dan durasi sesi yang Anda inginkan.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-3xl shadow-xl border border-slate-100/50 space-y-4">
                            <h4 className="font-bold text-slate-800 text-sm tracking-wide uppercase">Derajat Kecocokan Paket (Relevansi)</h4>
                                    ...
                                    return (
                                        <div key={pkg.id} className="space-y-1.5">
                                            <div className="flex justify-between text-xs sm:text-sm font-semibold">
                                                <span className={isBest ? "text-brand-deep font-bold flex items-center gap-1.5" : "text-slate-500"}>
                                                    {pkg.name}
                                                    {isBest && <span className="text-3xs bg-emerald-500/10 text-emerald-600 px-2 py-0.5 rounded-full border border-emerald-200">Terbaik</span>}
                                                </span>
                                                <span className={isBest ? "text-brand-primary font-bold" : "text-slate-400"}>{score}%</span>
                                            </div>
                        ...
                        {(() => {
                            const bestPkg = PACKAGES.find((p) => p.id === bestMatch) || PACKAGES[0];
                            return (
                                <div className={`bg-gradient-to-r ${bestPkg.bgGradient} rounded-3xl text-white shadow-2xl p-6 sm:p-8 relative overflow-hidden transition-all duration-300`}>
                                    ...
                                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/10 pb-6 mb-6">
                                        <div>
                                            <h3 className="text-2xl font-extrabold tracking-tight">{bestPkg.name}</h3>
                                            <p className="text-xs text-slate-300/90 mt-1">{bestPkg.desc}</p>
                                        </div>
                                        <div className="text-left sm:text-right">
                                            <span className="text-3xs uppercase tracking-widest text-slate-300 font-bold">Harga Paket</span>
                                            <h4 className={`text-2xl font-black ${bestPkg.textColor}`}>{bestPkg.price}</h4>
                                        </div>
                                    </div>
 
                                    {/* Features Checklist */}
                                    <div className="space-y-3 mb-8">
                                        <p className="text-xs uppercase tracking-widest text-slate-300 font-bold mb-4">Termasuk Layanan</p>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            {bestPkg.features.map((feature, idx) => (
                                                <div key={idx} className="flex items-start gap-2.5">
                                                    <div className={`bg-white/10 p-1 rounded-md ${bestPkg.textColor} mt-0.5`}>
